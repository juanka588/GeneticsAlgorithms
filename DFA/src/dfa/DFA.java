...
public class DFA {
    ...
    public static void test(String[] args) throws IOException {
        ...
        if (nRep < 2) {
            ArrayList<SimpleHaea> smIndividuals = new ArrayList<>();
            for (int i = 0; i < pop.size(); i++) {
                SimpleHaea<AgentDFA, AgentDFA> sm = new SimpleHaea<>(i, function, pop, grow, MAXITER, opers);
                smIndividuals.add(sm);
            }
            Random rn = new Random();
            for (int i = 0; i < MAXITER; i++) {
                for (int j = 0; j < pop.size(); j++) {
                    smIndividuals.get(j).iterar();
                }
                /*
                 insertar inmigrante ilegal
                 */
                if (i % 100 == 0) {
                    int change = rn.nextInt(pop.size());
                    p = pop.get(change).newInstance((AgentDFA) ikey.get(array));
                    pop.set(change, p);
                }
                System.out.println(estatPop(smIndividuals, i));
            }
            int idxBest = 0;
            double bestF = smIndividuals.get(idxBest).fitness;
            for (int i = 0; i < pop.size(); i++) {
                double f = smIndividuals.get(i).fitness;
                System.out.println(f + " vecindad " + smIndividuals.get(i).vecindad);
                if (f > bestF) {
                    bestF = f;
                    idxBest = i;
                }
            ...
            System.out.println("");
            System.out.println(pop.get(idxBest).get());
            System.out.println(smIndividuals.get(idxBest).fitness);
            examples = readExamples(new File("./datasets/test-" //+ "10.txt"));
                    + (nStates - resta) + ".txt"));
            function = new FitnessDFA(examples, POPSIZE, posEti, true);
            System.out.println(pop.get(idxBest).evaluate(function));
        } else {
            ...
            for (int k = 0; (k < nRep); k++) {
                ...
                if (k % 2 != 0) {
                    ...
                    opers = new Operator[]{mutation, sMutation, transposition};
    ...
    private static String estatPop(ArrayList<SimpleHaea> smIndividuals, int generation) {
        double max = Double.NEGATIVE_INFINITY, min = Double.POSITIVE_INFINITY, avg = 0, des = 0;
        for (int i = 0; i < smIndividuals.size(); i++) {
            if (smIndividuals.get(i).fitness > max) {
                max = smIndividuals.get(i).fitness;
            }
            if (smIndividuals.get(i).fitness < min) {
                min = smIndividuals.get(i).fitness;
            }
            avg += smIndividuals.get(i).fitness;
        }
        avg = avg / smIndividuals.size();
        for (int i = 0; i < smIndividuals.size(); i++) {
            des += Math.pow((smIndividuals.get(i).fitness - avg), 2);
        }
        des = des / (smIndividuals.size() - 1);

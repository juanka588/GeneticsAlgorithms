 ...
 * @author Camiku
 */
public class Parser implements FplConstants {

    private static int type_variable = -1, n, i;
    private static Token token = null, tokenException = null;
    private static Term child = null;
    private static Term parent = null;
    private static LinkedList<Byte> stackArgs = new LinkedList();
    private static LinkedList<Term> listTerm = null;
    private static LinkedList<Token> listToken = null;
    private static LinkedList<Byte> stack = new LinkedList();
    ...
    public static LinkedList<Term> parsing(String source) throws SyntacticalException, LexicalException {
        ...
        try {
            listToken = Scanner.generateTokens(source);
            stackArgs.clear();
            Term node = new Term();
            Term root = node;
            stack.clear();
            ...
            do {
                node = computational_step(listToken, stack, stringProccesed, node);
                if (node == null) {
                    for (Iterator<Token> it = listToken.iterator(); it.hasNext();) {
                        tokenException = it.next();
                        if(tokenException.getType() == CARRIAGE_RETURN
                            || tokenException.getType() == SEMICOLON
                            || tokenException.getType() == MARK_OF_BOTTOM){
                            break;
                        }
                        stringProccesed.append(tokenException.getValue());
                    }
                    throw new SyntacticalException(stringProccesed.toString());
                }
                switch (node.getType()) {
                    case EQUAL_SYMBOL:
                        node.setType(EQUAL);
    ...
    private static Term computational_step(LinkedList<Token> listToken,
            LinkedList<Byte> stack, StringBuilder stringProcessed, Term node) {
        ...
        switch (type_variable) {
            ...
            case CONSTANT:
                ...
                if (type_variable == token.getType()) {
                    ...
                    stringProcessed.append(token.getValue());
                } else {
                    stack = null;
                }
                break;
            ...
            case COMMA:
                ...
                if (node.getParent() != null
                    ...
                    if (node.getType() == LIST) {
                        ...
                        child.setParent(node);
                        node.addChild(child);
                        stackArgs.push((byte) (stackArgs.pop().byteValue() + 1));
                    }
                    stringProcessed.append(token.getValue());
                    return child;
                } else {
            ...
            case CARRIAGE_RETURN:
                token = listToken.pop();
                if (type_variable == token.getType()) {
                    stringProcessed.delete(0, stringProcessed.length());
    ...
    private static Term generateTreeNumber(int n, Term root) {
        if (n == 0) {
            ...
            root.setValue('s');
            Term node = new Term("0", NATURAL, null);
            for (i = 1; i < n; i++) {
